...
export class CreateVentasComponent implements OnInit {
  ...
  func_filtro_producto(){
    ...
    if(this.filtro_producto){
      ...
      this.inventario = this.variedades_const.filter(item=>term.test(item.producto));
    }else{
      this.inventario = this.variedades_const;
    }
  }
  ...
  init_direcciones(id:any){
    ...
    this._direccionService.getByClient(id).subscribe(
      response=>{
  ...
  init_productos(){
    ...
    // this._variedadService.getWithProductInfo().subscribe(
    //   response=>{
      ...
      if(this.cantidad >= 1){
        if(this.cantidad <= this.producto_select.cantidad){
          this.dventa.push({
            ...
            precio_und: this.producto_select.precio_mexicanos,
            titulo_variedad: this.producto_select.variedad,
            producto: this.producto_select.idproducto,
            subtotal: this.producto_select.precio_mexicanos * this.cantidad,
            inventario: this.producto_select.idInventario,
            cantidad: this.cantidad
          });
          
          this.total_pagar = this.total_pagar + (this.producto_select.precio_mexicanos * this.cantidad);
          this.neto_pagar = this.neto_pagar + (this.producto_select.precio_mexicanos * this.cantidad);
        }else{
          iziToast.show({
              title: 'ERROR',
              titleColor: '#FF0000',
  ...
  registrar_venta(){
    ...
    if(!this.venta.cliente){
      ...
      this.load_btn = true;
      this._ventaService.insert(this.venta).subscribe(
        response=>{
          this.load_btn = false;
